...
| 文档与交接 | 84% | 进行中 | PRD、任务清单、开发拆解和本文件已存在；本文件已同步最近 UI 与信息架构调整，并记录了远端仓库和最近一次推送。 |
...
| P0 | 进行中 | 启动 `npm run dev`，检查 `overview`、`chat`、`today`、`inventory`、`weekly`、`nutrition`、`shopping` 页面 | 验证主要页面无明显溢出、遮挡或交互异常；本次代码已推送，下一步可围绕 UI 验收或真实数据接入继续。 |
