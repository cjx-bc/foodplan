...
async function main() {
  ...
  const chatResult = await request(`/conversations/${conversationId}/messages`, {
    method: "POST",
    body: JSON.stringify({
  ...
  const mealPlanDetail = await request(`/meal-plans/${mealPlan.id}`);
  assert((mealPlanDetail.data as { id: string }).id === mealPlan.id, "meal plan detail mismatch");

  const regenerated = await request(`/meal-plans/${mealPlan.id}/meals/lunch/regenerate`, {
    method: "POST",
    body: JSON.stringify({
      reason: "提升蛋白质",
      message: "午餐换一个",
  ...
  const currentShopping = await request(`/shopping-lists/current?sourceType=meal_plan&sourceId=${mealPlan.id}`);
  ...
  await request(`/shopping-lists/${currentShoppingData.id}/items/${currentShoppingData.items[0].id}`, {
    method: "PATCH",
    body: JSON.stringify({ checked: !currentShoppingData.items[0].checked }),
  });

  const weekly = await request("/weekly-plans", {
    method: "POST",
    body: JSON.stringify({
      message: "本周尽量清淡，高蛋白。",
  ...
  const adjusted = await request(`/weekly-plans/${weeklyPlan.id}/days/${weeklyPlan.days[0].date}`, {
    method: "PATCH",
    body: JSON.stringify({
      message: "微调第一天晚餐",
      replaceMeals: ["dinner"],
  ...
  const adopted = await request(`/weekly-plans/${weeklyPlan.id}/adopt`, {
    method: "POST",
  ...
  await request(`/shopping-lists/current?sourceType=weekly_plan&sourceId=${weeklyPlan.id}`);
  await request(`/meal-plans/${adoptData.syncedMealPlanId}`);
